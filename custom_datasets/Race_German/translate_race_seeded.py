...
def build_qa_text(article_de: str, problems: Sequence[Dict]) -> str:
    """Optional readable QA field; not used by lm-eval but handy for inspection."""
    blocks = [f"Lesen Sie den folgenden Text und beantworten Sie die Fragen.\n\n{article_de}"]
    for idx, problem in enumerate(problems, 1):
        opts = problem.get("options", [])
        opts_serialized = "[" + ", ".join(opts) + "]"
        blocks.append(
            f"Frage {idx}: {problem.get('question', '')}\n"
            f"Antwortmoeglichkeiten: {opts_serialized}\n"
            f"Antwort: {problem.get('answer', '')}"
        )
    return "\n\n".join(blocks)
...
def main():
    ...
    default_output_dir = repo_root / "custom_datasets" / "Race_German"
    ...
    train_out = args.output_dir / "race_de_train.csv"
    test_out = args.output_dir / "race_de_test.csv"
