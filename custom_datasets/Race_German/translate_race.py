"""
Translate a seeded sample from RACE train/test splits into a chosen language.
...
LANG_CONFIG = {
    "german": {
        "lang_code": "deu_Latn",
        "short_code": "de",
        "qa_intro": "Lesen Sie den folgenden Text und beantworten Sie die Fragen.",
    },
    "hindi": {
        "lang_code": "hin_Deva",
        "short_code": "hi",
        "qa_intro": "कृपया निम्नलिखित पाठ पढ़ें और प्रश्नों के उत्तर दें।",
    },
    "portuguese": {
        "lang_code": "por_Latn",
        "short_code": "pt",
        "qa_intro": "Leia o texto a seguir e responda às perguntas.",
    },
    "french": {
        "lang_code": "fra_Latn",
        "short_code": "fr",
        "qa_intro": "Lisez le texte suivant et répondez aux questions.",
    },
    "italian": {
        "lang_code": "ita_Latn",
        "short_code": "it",
        "qa_intro": "Leggi il seguente testo e rispondi alle domande.",
    },
    "spanish": {
        "lang_code": "spa_Latn",
        "short_code": "es",
        "qa_intro": "Lee el siguiente texto y responde las preguntas.",
    },
}
...
def translate_text(text: str, tokenizer, model, forced_bos_token_id: int) -> str:
...
def build_qa_text(article_tgt: str, problems: Sequence[Dict], qa_intro: str) -> str:
    blocks = [f"{qa_intro}\n\n{article_tgt}"]
    for idx, problem in enumerate(problems, 1):
        opts = problem.get("options", [])
        opts_serialized = "[" + ", ".join(opts) + "]"
        blocks.append(
            f"Question {idx}: {problem.get('question', '')}\n"
            f"Answer choices: {opts_serialized}\n"
            f"Answer: {problem.get('answer', '')}"
        )
    return "\n\n".join(blocks)
...
def translate_split(
    ...
    for idx, example in enumerate(progress):
        article_tgt = translate_text(example["article"], tokenizer, model, forced_bos_token_id)

        problems: List[Dict] = []
        for question, opts, answer in zip(
            example["questions"], example["options"], example["answers"]
        ):
            q_tgt = translate_text(question, tokenizer, model, forced_bos_token_id)
            opts_tgt = [translate_text(opt, tokenizer, model, forced_bos_token_id) for opt in opts]
            problems.append({"question": q_tgt, "answer": answer, "options": opts_tgt})

        qa_text = build_qa_text(article_tgt, problems, qa_intro)
        rows.append(
            {
                "id": example["id"],
                "article": article_tgt,
...
def resolve_target_language(target_language: str) -> Tuple[str, str, str]:
    ...
    if key not in LANG_CONFIG:
        supported = ", ".join(sorted(LANG_CONFIG.keys()))
        raise ValueError(f"Unsupported target language '{target_language}'. Supported: {supported}")
    cfg = LANG_CONFIG[key]
    return cfg["lang_code"], cfg["short_code"], cfg["qa_intro"]


def main():
    repo_root = Path(__file__).resolve().parents[2]
    default_data_root = repo_root / "custom_datasets" / "Race_German" / "RACE"
    default_output_dir = repo_root / "custom_datasets" / "Race_French"

    parser = argparse.ArgumentParser(description="Translate a seeded sample of RACE to a target language.")
    parser.add_argument("--target_language", type=str, default="french", help="german|hindi|portuguese|french|italian|spanish")
    ...
    target_lang_code, target_short_code, qa_intro = resolve_target_language(args.target_language)
    ...
    train_out = args.output_dir / f"race_{target_short_code}_train.csv"
    test_out = args.output_dir / f"race_{target_short_code}_test.csv"

    print(f"Target language: {args.target_language} ({target_lang_code})")
    ...
    translate_split(sampled_train, tokenizer, model, forced_id, train_out, args.save_every, qa_intro)

    print(f"Translating {len(sampled_test)} test rows -> {test_out}")
    translate_split(sampled_test, tokenizer, model, forced_id, test_out, args.save_every, qa_intro)
