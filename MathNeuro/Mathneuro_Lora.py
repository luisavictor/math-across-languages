...
parser.add_argument(
    '--fine_tune_lora',
    help="if set, use LoRA fine-tuning on isolated task-specific parameters instead of pruning/scaling",
    action="store_true"
)
parser.add_argument(
    '--lora_r',
...
parser.add_argument(
    '--lora_epochs',
    help="number of epochs for LoRA fine-tuning",
    type=int,
    default=1
)
parser.add_argument(
...
parser.add_argument(
    '--lora_max_length',
    help="max sequence length for LoRA fine-tuning",
    type=int,
    default=1024
)
# end LoRA args
...
# ---- NEW: make sure we have a pad token ----
if tokenizer.pad_token is None:
    # Option 1 (simple): reuse eos as pad
...
# Make model aware of the pad token
...
def get_lora_target_modules_from_mask(mask_dict):
    """
    Use the task-specific mask to decide which module names to attach LoRA to.
    We look at parameter names like 'model.layers.0.self_attn.q_proj.weight'
    and collect the module name just before '.weight' (e.g. 'q_proj').
    """
    target_modules = set()
    for name in mask_dict.keys():
        ...
        if len(parts) < 2:
            continue
        module_name = parts[-2]  # e.g. 'q_proj'
...
class PromptDataset(Dataset):
    def __init__(self, df, train_df_full, tokenizer):
        """
        df: sampled_train or sampled_comparison used for finding good params
        train_df_full: full train DataFrame (for few-shot formatting if needed)
        tokenizer: HF tokenizer (unused in __getitem__, but kept for extension)
        """
        self.prompts = []
        # We use the same formatting you use in find_good_params
        for i in range(len(df)):
            if 'qa' in df.columns.to_list():
                prompt = df.iloc[i]['qa']
            else:
                question = df['question'].iloc[i]
                answer = df['solution'].iloc[i]
                prompt = f"""Instruct: {question} Let's write a Python program.\nOutput:\n{answer}"""
...
def fine_tune_lora(
    ...
    """
    Fine-tune the model with LoRA on task-specific modules derived from good_params.
    """
    device = model.device

    # 1) Derive target_modules from your task-specific mask
    target_modules = get_lora_target_modules_from_mask(good_params)
    if len(target_modules) == 0:
        # Fallback to typical LM target modules if mask is empty
    ...
    dataset = PromptDataset(train_df, train_df, tokenizer)
    ...
    def collate_fn(batch):
        ...
        enc = tokenizer(
            ...
            padding=True,
            truncation=True,
            max_length=args.lora_max_length
        )
    ...
    for repeat in range(0, num_repeats):
        ...
        for good_percent in good_percents:
            ...
            if args.fine_tune_lora:
                # LoRA fine-tuning on task-specific parameters instead of pruning/scaling
                # Here we use the task-specific mask 'good_params' to decide which modules
                # get LoRA adapters, and fine-tune on the sampled_train data.
                ...
                # We keep the base weights intact and only adapt via LoRA.
                del good_params
                del comparison_params
            else:
                # Original behavior: pruning (scalar=0) or scaling (scalar!=0)
