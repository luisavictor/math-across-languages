...
parser.add_argument(
    '--data_mode',
    choices=['original', 'correct_only', 'false_only'],
    default='original',
    help="training data mode for parameter selection: use all samples, only lm_eval-correct, or only lm_eval-incorrect"
)
parser.add_argument(
    '--correct_train_dataset',
    type=str,
    default=None,
    help="path to precomputed lm_eval-correct training subset CSV (used when data_mode=correct_only)"
)
parser.add_argument(
    '--false_train_dataset',
    type=str,
    default=None,
    help="path to precomputed lm_eval-incorrect training subset CSV (used when data_mode=false_only)"
)
...
train = pd.read_csv(args.train_dataset).reset_index(drop=True)
...
if args.data_mode != "original":
    if args.data_mode == "correct_only":
        selected_path = args.correct_train_dataset
        if selected_path is None:
            filtered_output_dir = Path(args.save_path) / "eval_results" / args.model / "filtered_training_data"
            matches = sorted(filtered_output_dir.glob("train_correct*.csv"))
            if len(matches) == 1:
                selected_path = str(matches[0])
            elif len(matches) == 0:
                raise ValueError(
                    "data_mode=correct_only requires --correct_train_dataset or exactly one "
                    f"'train_correct*.csv' in {filtered_output_dir}"
                )
            else:
                raise ValueError(
                    "Multiple correct subset files found. Pass --correct_train_dataset explicitly. "
                    f"Found: {[str(m) for m in matches]}"
                )
    else:
        selected_path = args.false_train_dataset
        if selected_path is None:
            filtered_output_dir = Path(args.save_path) / "eval_results" / args.model / "filtered_training_data"
            matches = sorted(filtered_output_dir.glob("train_incorrect*.csv"))
            if len(matches) == 1:
                selected_path = str(matches[0])
            elif len(matches) == 0:
                raise ValueError(
                    "data_mode=false_only requires --false_train_dataset or exactly one "
                    f"'train_incorrect*.csv' in {filtered_output_dir}"
                )
            else:
                raise ValueError(
                    "Multiple incorrect subset files found. Pass --false_train_dataset explicitly. "
                    f"Found: {[str(m) for m in matches]}"
                )

    train = pd.read_csv(selected_path).reset_index(drop=True)
    print(f"Using filtered training subset from {selected_path}: {len(train)} rows")

train = train.sample(frac=1, random_state=args.random_state).reset_index(drop=True)
